...
def add_user_order(request):
    ...
    if new_order_form.is_valid():
        ...
        ordering = order.orderdetail_set.create(product_id=product.id, price=product.price, count=count)
        t = threading.Thread(target=longTask, args=[ordering])
        t.setDaemon(True)
        t.start()
